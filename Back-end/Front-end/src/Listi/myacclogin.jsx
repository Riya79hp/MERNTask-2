...
const LoginForm = () => {
  ...
  useEffect(() => {
    ...
    if (query) {
      console.log(query);
      axios.post('http://localhost:4500/myacc/signup/authenticateuser', { query})
        .then((res) => {
  ...
  const handleSubmit = (event) => {
    ...
    axios.post(`http://localhost:4500/myacc`, { username, password })
      .then((res) => {
        if (res.data.result === 'success') {
  ...
  const handleForgotPassword = async () => {
    ...
    if (uname) {
      const response = await axios.post(`http://localhost:4500/myacc/forgotpassword`, { uname });
      if (response.data === "nouser") {
        alert('User does not exist');
      } else {
        let newPassword = prompt('Enter new password');
        if (newPassword) {
          const resetResponse = await axios.post(`http://localhost:4500/myacc/forgotpassword/reset`, { uname, password: newPassword });
